...
# Note: The RSI thresholds are now in the conventional 0-100 range,
# and we add a new "macd_threshold" parameter.
param_grid = {
    "rsi_period": [10, 12, 14, 16],
    "rsi_threshold_low": [0.30, 0.32, 0.34],
    "rsi_threshold_high": [0.68, 0.70, 0.72],
    "bb_period": [20],
    "k": [2.47],
    "bb_width_threshold": [0.0010],
    "macd_threshold": [0.5, 1.0, 1.5, 2.0],
    "atr_period": [9],
...
for params in grid:
    ...
    # Create the instrument and bar type.
    SPY = TestInstrumentProvider.equity(symbol="SPY", venue="XNAS")
    timeframe = "1-HOUR"
    spy_bar_type = BarType.from_str(f"{SPY.id}-{timeframe}-LAST-EXTERNAL")

    # Build strategy configuration using new grid parameters.
    bbrsi_config = BBRSIStrategyConfig(
        ...
        macd_fast_period=26,  # Fixed as best known value.
        macd_slow_period=52,  # Fixed as best known value.
        macd_signal_period=9,  # Unused.
